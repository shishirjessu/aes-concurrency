...
for i in range(len(sizes)):
    ...
    temp = []
    for _ in range(10):
        temp.append(float(check_output((rules[0] + types[0] + size), shell=True).decode()))
    seq_times_c.append(sum(temp)/10)

    temp = []
    for _ in range(10):
        temp.append(float(check_output((rules[0] + types[1] + size), shell=True).decode()))
    par_times_c.append(sum(temp)/10)

    temp = []
    for _ in range(10):
        temp.append(float(check_output((rules[1] + types[0] + size), shell=True).decode()))
    seq_times_go.append(sum(temp)/10)

    temp = []
    for _ in range(10):
        temp.append(float(check_output((rules[1] + types[1] + size), shell=True).decode()))
